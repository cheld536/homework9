...
{
	Node* newnode = (Node*)malloc(sizeof(Node)); // 사용자로부터 새로 입력 받을 노드를 동적할당해준다. 

	newnode->key = key;							 // 노드를 입력받은 키값으로 초기화 해준다		
	newnode->left = NULL;						 // 노드의 왼쪽 루트 가르키는 값을 널로 초기화
	newnode->right = NULL;						 // 노드의 오른쪽 루트가 가르키는 값을 널로 초기화	
	...
	while (findnode != NULL)			// NULL이 아닐때 까지 반복
	{
		if (findnode->key <= key)				// 입력값이 더 클때
		{
			...
			{
				findnode->right = newnode;		// 오른쪽에 입력한 노드를 위치 시킨다.
				break;
			}
		...
		else								// 부모노드보다 작거나 같으면 
		{
			...
			{
				findnode->left = newnode;		// 왼쪽에 입력한 노드를 위치 시킨다.
				break;
			}
...
int deleteLeafNode(Node* head, int key)
{
	...
	Node* parents = NULL;
	Node* remove = NULL;
	while (target != NULL)								// 타겟이 널이 아닐때 까지 반복
	{

		if (key == target->key)							// 값을 찾았으면 반복문 중지
			break;
		if (key < target->key)							// 입력된 값보다 찾는 값이 더 크면 
		{
			parents = target;							// 부모노드에 탐색노드의 주소를 저장하고
			target = target->left;						// 탐색노드는 왼쪽으로 이동
		}
		else if (key > target->key)						// 입력된 값이 찾는 값보다 클때 
		{
			parents = target;							// 부모노드에 탐색노드의 주소를 저장하고
			target = target->right;						// 탐색노드는 오른쪽으로 이동
		}

	}

	remove = target;									// 노드를 찾고 삭제할 노드의 주소를 저장
	if (target == NULL)									// 노드가 없을 경우
	{
		printf("삭제하려는 %d 노드가 없습니다.\n", key);			
		return 0;
	}
	if (target->left != NULL && target->right != NULL)			// 만약 자식노드가 존재할때
	{
	...
	{
		if (target->key == parents->left->key)						// 찾는 값과 부모 노드의 왼쪽의 값이 같을때
		{
			free(remove);											// 자식 노드를 메모리 해제하고
			parents->left = NULL;									// 왼쪽을 널값으로 초기화 한다.
		}
		else if (target->key == parents->right->key)				// 찾는 값과 부모 노드의 오른쪽 값이 같을때 
		{	
			free(remove);											// 자식노드를 메모리 해제하고
			parents->right = NULL;									//오른쪽을 넓값으로 초기화 한다.
		}
	}
...
{
	Node *target = head->left;
	while (target)										// 헤드가 널이 아닐때까지 반복
	{
		if (key == target->key) return target;			// key값을 찾으면 찾은 노드의 주소를 반환
		if (key < target->key)							// 입력된 key값보다 탐색된 key값이 더 크면
		{
			target = target->left;						// 왼쪽으로 이동
		}
		else										// 아니면 오른쪽으로 이동 
		{
			target = target->right;
